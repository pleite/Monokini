...
Bridge_Channel_Length_in_Units_Y_Axis_Bottom = 1;
//Number of grids extending along the Y axis from the corner grid in units to the top (default unit is 28mm)
Bridge_Channel_Length_in_Units_Y_Axis_Top = 1;
//Grid units transition (Y axis, equal both sides)
Units_Over = 1; //[0:0.5:10]
//Grid units bridge size (Y axis)
Units_Up = 1; //[1:1:10]
//Bridge height (Z axis) in mm
Bridge_Height = 22; //[0:6:72]
...
lengthMM = Bridge_Channel_Length_in_Units_Y_Axis_Bottom * Grid_Size;
//lengthMM2 = X_Channel_Length_in_Units_X_Axis_Right * Grid_Size;
//lengthMM3 = X_Channel_Length_in_Units_X_Axis_Left * Grid_Size;
lengthMM2 = Bridge_Channel_Length_in_Units_Y_Axis_Top * Grid_Size;
Bridge_Transition = Units_Over * Grid_Size; //distance from the bottom of the channel to the top of the channel
Bridge_Length = Units_Up * Grid_Size; //distance from the bottom of the channel to the top of the channel
baseHeight = 3.40;
topHeight = 18.60;
...
//BEGIN BEZ ATTEMPT
dx = 0; // Units_Over * Grid_Size;
dy = ((Units_Up+(Units_Over*2))-1) * Grid_Size;


//t = bezpath_curve(bez);

t =([[0,-lengthMM-Grid_Size,0],[0,-Grid_Size,0],[0, Bridge_Transition-Grid_Size,-Bridge_Height],[0, Bridge_Transition+Bridge_Length-Grid_Size,-Bridge_Height],[0, dy,0],[0, dy+lengthMM2,0]]);
...
color_this(Global_Color) 
  monokiniChannel(lengthMM = lengthMM, widthMM = channelWidth, heightMM = Channel_Total_Height, anchor = CENTER, orient = TOP, spin = 0, suppress = Suppress_List[0], supressWalls = true)
...
back(lengthMM2/2+dy)
color_this(Global_Color) 
  monokiniChannel(lengthMM = lengthMM2, widthMM = channelWidth, heightMM = Channel_Total_Height, anchor = CENTER, orient = TOP, spin = 0, suppress = Suppress_List[1], supressWalls = true);
...
color_this(Global_Color)
monokiniChannelPath3d(path = t, widthMM = channelWidth, heightMM = Channel_Total_Height, anchor = CENTER, orient = TOP, spin = 0);
}

fwd(lengthMM/2+Grid_Size)
color_this(Global_Color) 
monokiniChannelCuts(lengthMM = lengthMM, widthMM = channelWidth, heightMM = Channel_Total_Height, anchor = CENTER, orient = TOP, spin = 0, suppress = Suppress_List[0]);

left(-dx)
back(lengthMM2/2+dy)
color_this(Global_Color) 
  monokiniChannelCuts(lengthMM = lengthMM2, widthMM = channelWidth, heightMM = Channel_Total_Height, anchor = CENTER, orient = TOP, spin = 0, suppress = Suppress_List[1]);
...
//left(Grid_Size) 
  color_this("red") 
  monokiniChannel(lengthMM = 2*Grid_Size+channelWidth, widthMM = Bridge_Length, heightMM = Channel_Total_Height, anchor = CENTER, orient = TOP, spin = 90, suppress = "BBBBBBBB");
...
module monokiniChannel(lengthMM = 28, widthMM = 26.4, heightMM = 22, anchor, spin, orient, suppress = "", supressWalls = false) {
...
module monokiniChannelCuts(lengthMM = 28, widthMM = 26.4, heightMM = 22, anchor, spin, orient, suppress = "") {
...
/*
 ...
 module monokiniChannelCurve(lengthMM = 28, widthMM = 26.4, heightMM = 22, anchor, spin, orient) {
    ...
    roff_Profile = offset(monokiniProfile, delta=-snapWallThickness, check_valid=true); //create the openGrid profile
    point1 = select(monokiniProfile, 0); //get the first point of the profile
    point2 = select(roff_Profile, 0); //get the first point of the offset profile
    point3 = select(roff_Profile, -1); //get the last point of the offset profile
    roff_Profile2 = list_set(roff_Profile, 0, [point2[0], point1[1]]); //fix the first point to be the same as the original profile
...
module monokiniChannelPath3d(path, lengthMM = 28, widthMM = 26.4, heightMM = 22, anchor, spin, orient) {
...
supportProfile = [
 ...
 //   left_half(s = max(widthMM*2, heightMM*2), x=Nudge) 
    path_extrude(path)
    yflip() polygon(off_Profile2);
    if (Decorative) {
    path_extrude(path) xflip()
        xcopies(l=widthMM-topChamfer*2-Decorative_Height*2-minWall*2, spacing=Support_Bridging_Separation)
        polygon(supportProfile);
    }
    } children();
