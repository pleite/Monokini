...
Label_Width_in_Units = 1;  // Ensure this is an integer
//Length (Y axis) of channel in units. Default unit is 28mm
Label_Length_Units = 5; 
...
// Grip increase clearance in mm
Grip_Increase_Clearance = 0; // [0:0.05:0.5]
...
channelWidth = (Grid_Size-channelWidthSeparation*2) + (Label_Width_in_Units-1) * Grid_Size;
lengthMM = Label_Length_Units * Grid_Size;
baseHeight = 3.40;
topHeight = 18.60;
...
color_this(Global_Color) diff() {
  ...
  monokiniPlate(lengthMM = lengthMM, widthMM = channelWidth, heightMM = Channel_Total_Height, anchor = CENTER, orient = TOP, spin = 0, suppress = Suppress_List[0]) //str_pad(Suppress_List[0], Label_Length_Units, "N")
...
module monokiniGrip(widthMM = 26.4, heightMM = 22) {
 ...
 monokiniProfileGrip =
    ...
    gripPath = [[Grip_Increase_Clearance-1*widthMM/2,spacingFromChannel],[Grip_Increase_Clearance-1*widthMM/2,gripSize+spacingFromChannel]];
...
module monokiniPlate(lengthMM = 28, widthMM = 26.4, heightMM = 22, anchor, spin, orient, suppress = "") {
    ...
    monokiniProfile = [
            [-1*widthMM/2, 0],
            [-1*widthMM/2, snapWallThickness],
            [widthMM/2, snapWallThickness],
            [widthMM/2, 0]
        ];
...
*/
        ...
        path_copies(pathChannel, spacing=Grid_Size) {
            ...
            // echo("Index: ", $idx, " Suppress: ", suppress[$idx]);
                if (suppress[$idx] != "B" && suppress[$idx] != "L")
                    //if (Grip_Flare > 0)
                    //    monokiniGripFlared(widthMM = widthMM, heightMM = snapWallThickness, flareAngle = Grip_Flare);
                    //else
                        monokiniGrip(widthMM = widthMM, heightMM = snapWallThickness);
                if (suppress[$idx] != "B" && suppress[$idx] != "R")
                    //if (Grip_Flare > 0)
                    //    xflip() monokiniGripFlared(widthMM = widthMM, heightMM = snapWallThickness, flareAngle = Grip_Flare);
                    //else
                        xflip() monokiniGrip(widthMM = widthMM, heightMM = snapWallThickness);
            }
        }
    /*
    }
    if (lengthMM > gripSize)
        path_copies(pathChannel, spacing=Grid_Size) {
            left(Grid_Size/2)
            zrot(90) 
            down(22-snapWallThickness) {
            // echo("Index: ", $idx, " Suppress: ", suppress[$idx]);
                if (suppress[$idx] != "B" && suppress[$idx] != "L")
                    if (Grip_Flare > 0)
                        monokiniGripFlaredCut(widthMM = widthMM, heightMM = snapWallThickness, flareAngle = Grip_Flare);
                    // else
                        // monokiniGrip(widthMM = widthMM);
                if (suppress[$idx] != "B" && suppress[$idx] != "R")
                    if (Grip_Flare > 0)
                        xflip() monokiniGripFlaredCut(widthMM = widthMM, heightMM = snapWallThickness, flareAngle = Grip_Flare);
                    // else
