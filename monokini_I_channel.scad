...
//Supression string Key: N - None, L - Left suppressed, R - Right suppressed, B - Both suppressed, S - Static grip (ignore flaring). With multiple channels, separate them with comma.
Suppress_Connectors = ""; // 
// Decorative option
...
/*[Filament Attachment Options]*/
Filament_Show = false; // [true, false]
Filament_Diameter = 1.75;  // [0:0.01:10]
Filament_Depth = 0.00; // [-50:0.01:50]
Filament_Offset = 0.00;  // [-50:0.01:50]
Filament_Repeat = 1;
Filament_Spacing = 5.00;  // [0:0.01:50]
...
if (Filament_Show)
down(Filament_Depth) 
    for (i = [0 : Filament_Repeat - 1]) {
        left(-((Filament_Repeat - 1) * Filament_Spacing) / 2 + i * Filament_Spacing + Filament_Offset)
            back(lengthMM-Nudge/2) xrot(90) linear_extrude(height=lengthMM+Nudge) 
                teardrop2d(d=1.75);
    }
